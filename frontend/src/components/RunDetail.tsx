...
/** The current-route label always follows the persisted Run strategy, never a literal. */
function runImageVersion(strategy: string | null | undefined): string {
...
/**
 * Project the authoritative Image 5.0 run-level cover palette evidence from
 * its persisted top-level location. Image 5.0 has no Position-2 seed lineage;
 * the 3.0 seed_palette_lineage shape is not interchangeable with this
 * cover_palette result, and private paths, prompts and raw failure text stay
 * out of the public projection.
 */
const PUBLIC_COVER_PALETTE_FIELDS = [
  'source',
  'applied',
  'status',
  'error_code',
  ...
  'validated_xml_content_length',
] as const;

function publicCoverPaletteEvidence(
  run: { strategy?: string | null; stage_artifacts?: unknown } | null | undefined,
): Record<string, unknown> | undefined {
  if (!run || run.strategy !== 'image_5_0') return undefined;
  const palette = asRecord(asRecord(run.stage_artifacts)?.cover_palette);
  if (!palette || palette.source !== 'cover_image') return undefined;

  const evidence: Record<string, unknown> = {};
  for (const key of PUBLIC_COVER_PALETTE_FIELDS) {
    const value = palette[key];
    if (value !== undefined && value !== null) evidence[key] = value;
  }
  ...
  const content = (
    <div className="run-detail-evidence-panel-body">
      <div className="run-detail-evidence-header">
        ...
        <Space size={6} wrap>{contextTags}<Tag>Image {runImageVersion(run.strategy)}</Tag>{statusTag(slide.status)}</Space>
      </div>
...
const RunDetailView: React.FC<RunDetailProps> = ({ runId }) => {
  ...
  const coverPaletteEvidence = publicCoverPaletteEvidence(run);
  const paletteEvidence = coverPaletteEvidence ?? seedEvidence;
  ...
  const imageVersion = runImageVersion(run.strategy);

  return (
    <div className="run-detail-page public-image-run-detail">
      <div className="page-toolbar run-detail-page-toolbar">
        <div>
          <div className="page-kicker"><span className={`run-status-dot ${run.status}`} />Image PPT {imageVersion} · Run #{run.id}</div>
          <h2>Run #{run.id} Detail</h2>
          <p className="toolbar-subtitle">Read-only Image {imageVersion} generation evidence.</p>
        </div>
        <Space wrap>
      ...
      <section className="run-route-flow-panel" aria-label={`Image ${imageVersion} Run Status`}>
        <div className="section-heading compact"><h3>Image {imageVersion} Run Status</h3>{statusTag(run.status)}</div>
        <Descriptions bordered size="small" column={{ xs: 1, sm: 2 }}>
          <Descriptions.Item label="Run ID">{run.id}</Descriptions.Item>
          ...
          <Descriptions.Item label="Route">Image {imageVersion}</Descriptions.Item>
      ...
      <section className="run-route-flow-panel" aria-label="Seed and palette lineage">
        <div className="section-heading compact"><h3>Reference Input Map</h3><Tag color="gold">Image {imageVersion}</Tag></div>
        <div className="evidence-section-stack">
