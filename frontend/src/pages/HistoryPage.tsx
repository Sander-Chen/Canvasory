...
/** The public History/Batch projections admit exactly the two supported routes. */
export type PublicStrategy = 'image_3_0' | 'image_5_0';

const PUBLIC_STRATEGIES: readonly PublicStrategy[] = ['image_3_0', 'image_5_0'];

function publicStrategy(value: unknown): PublicStrategy | null {
  return typeof value === 'string' && (PUBLIC_STRATEGIES as readonly string[]).includes(value)
    ? value as PublicStrategy
    : null;
}

/** Route and version labels always follow the persisted strategy, never a literal. */
function imageVersionLabel(strategy: PublicStrategy): string {
  return strategy === 'image_5_0' ? '5.0' : '3.0';
}

function imageRouteLabel(strategy: PublicStrategy): string {
  return `Image Route (${imageVersionLabel(strategy)})`;
}

/** Only fields emitted by the public Image run projection are accepted here. */
...
/** Only fields emitted by the public Image batch projection are accepted here. */
...
function toPublicRun(value: unknown): PublicRun | null {
  const record = objectValue(value);
  const strategy = publicStrategy(record?.strategy);
  if (!record || record.engine !== 'image' || !strategy) return null;
  const id = positiveInt(record.id);
...
/**
 * History lists both supported routes; the header names exactly what the
 * persisted records contain. An empty legacy history keeps the historical
 * 3.0 heading.
 */
function historyRouteSummary(batches: PublicBatch[]): { kicker: string; subtitle: string } {
  const strategies = new Set(batches.map((batch) => batch.strategy));
  const present = PUBLIC_STRATEGIES.filter((strategy) => strategies.has(strategy));
  if (present.length > 1) {
    const versions = present.map(imageVersionLabel);
    return {
      kicker: `Image Routes (${versions.join(' + ')})`,
      subtitle: `Read-only history for Image ${versions.join(' and Image ')} batches, runs, progress, and PNG artifacts.`,
    };
  }
  if (present.length === 1) {
    const version = imageVersionLabel(present[0]);
    return {
      kicker: imageRouteLabel(present[0]),
      subtitle: `Read-only history for Image ${version} batches, runs, progress, and PNG artifacts.`,
    };
  }
  return {
    kicker: imageRouteLabel('image_3_0'),
    subtitle: 'Read-only history for Image 3.0 batches, runs, progress, and PNG artifacts.',
  };
}
...
const HistoryPage: React.FC = () => {
            ...
            <div key={run.id} className="history-expanded-run-card">
              <div className="history-expanded-run-card-header">
                <div>
                  ...
                  <span>{imageRouteLabel(run.strategy)}</span>
                </div>
  ...
  const renderBatchContext = (record: PublicBatch) => (
    <div className="history-context-cell">
      ...
      <Space size={4} wrap>
        <Tag color="gold">{imageRouteLabel(record.strategy)}</Tag>
  ...
  const routeSummary = historyRouteSummary(batches);

  return (
    <div className="history-page">
      <div className="page-toolbar">
        <div>
          <div className="page-kicker"><span className="status-dot" />{routeSummary.kicker}</div>
          <h2>History</h2>
          <p className="toolbar-subtitle">{routeSummary.subtitle}</p>
        </div>
