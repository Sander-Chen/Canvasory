...
import { api } from '../api';
import type { PublicBatchDetail, PublicRun, PublicSlide, PublicStrategy } from './HistoryPage';
...
/** The public Batch projection admits exactly the two supported routes. */
...
function toPublicRun(value: unknown): PublicRun | null {
  ...
  if (!record || record.engine !== 'image' || !strategy || !id || !deckId || !configId || typeof record.status !== 'string') return null;
  return {
    id,
...
function toPublicBatch(value: unknown): PublicBatchDetail | null {
  ...
  if (!record || record.engine !== 'image' || !strategy || !id || !deckId || !configId || typeof record.status !== 'string' || !numbers.every((key) => typeof record[key] === 'number' && Number.isFinite(record[key])) || !Array.isArray(record.runs)) return null;
...
const BatchOverviewPage: React.FC = () => {
  ...
  const loadBatch = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (!detail) throw new Error('Batch is not available on the public Image surface');
  ...
  return (
    <div className="batch-overview-page">
      <div className="page-toolbar">
        <div>
          <div className="page-kicker"><span className="status-dot" />{imageRouteLabel(batch.strategy)}</div>
          <h2>Batch #{batch.id} History</h2>
          <p className="toolbar-subtitle">Inspect sibling runs, honest progress, and PNG artifacts from one Image {imageVersionLabel(batch.strategy)} batch.</p>
        </div>
      ...
      <Descriptions bordered column={{ xs: 1, md: 3 }} className="batch-context">
        ...
        <Descriptions.Item label="Route">{imageRouteLabel(batch.strategy)}</Descriptions.Item>
        ...
        <div className="batch-run-grid">
          {batch.runs.map((run) => (
            <button type="button" key={run.id} className={`batch-run-card ${selectedRun?.id === run.id ? 'selected' : ''}`} onClick={() => selectRun(run)}>
              ...
              <div className="batch-run-meta"><span>{imageRouteLabel(run.strategy)}</span><span>{run.config_name || `Config ${run.config_id}`}</span><span>{progressText(run)}</span></div>
      ...
      <section className="batch-overview-section selected-run-evidence">
        ...
        {selectedRun ? (
          <Card
            title={<Space wrap><Tag color={statusColorMap[selectedRun.status] || 'default'}>{selectedRun.status}</Tag><span>{imageRouteLabel(selectedRun.strategy)}</span></Space>}
            extra={<Space wrap>
