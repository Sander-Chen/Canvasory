...
Image PPTGen 5.0 is the public product. Historical Image routes are not
...
presentation. Image PPTGen 5.0 shows a page split first. You may revise and
confirm that split, then either delegate the whole Deck design to Auto or
describe and refine a whole-Deck creative direction. Generation starts only
after you explicitly confirm the final direction.
...
Review and revise the page split
    ↓
Explicitly confirm the page split
    ↓
Choose Auto or refine a whole-Deck creative direction
    ↓
Explicitly confirm the final direction
    ↓
Generate
    ↓
...
Split review, design-direction alignment, and generation are separate stages.
Nothing is generated when the page split is confirmed. A written direction can
be revised for as many conversational turns as needed; only the final complete,
unqualified confirmation starts one generation. Changing the page count does
not require resubmitting the source material.

The documented user journey is: install, submit material, review or revise the
split, confirm pagination, align and confirm one whole-Deck direction, generate,
open the static Preview, and download the ZIP.

## Previous 3.0 acceptance demo: frog life cycle
...
- State and refine one creative direction for the whole Deck before generation
...
- Traceable split, design-direction confirmation, and generation states
...
| Path | Responsibility |
| --- | --- |
| `skills/generate-image-presentation/` | Codex Skill: install follow-through, split confirmation, whole-Deck direction alignment, and generation flow |
