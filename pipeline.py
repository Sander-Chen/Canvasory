...
def _native_image_director_models_for_route(route: str, role: str) -> frozenset[str]:
    """Return the allowlisted Codex text-stage models for one Native route role."""
    from backend.services import model_profiles

    if role == "image_palette_extractor":
        return frozenset({"gpt-5.6-sol"})
    if route == model_profiles.NATIVE_IMAGE_5_0_ROUTE:
        return NATIVE_IMAGE_3_0_DIRECTOR_MODELS
    return _native_image_3_0_director_models()
...
_PROMPT_SLOT_RE = re.compile(r"\{\{([^{}]+)\}\}")


def render_template_string_single_pass(template_content: str, variables: dict[str, str]) -> str:
    """Substitute {{variable}} placeholders in one left-to-right pass.

    ``render_template_string`` applies one ``str.replace`` per variable, so a
    value inserted by an earlier variable is rescanned and rewritten by every
    later variable.  The Image 5.0 unified body template inserts the confirmed
    whole-Deck Requirement verbatim, and a Requirement that itself contains a
    placeholder token must survive untouched, so that role renders with this
    renderer.  Slots absent from ``variables`` are left as written.
    """
    ...
    return _PROMPT_SLOT_RE.sub(_substitute, template_content)


IMAGE_5_0_UNIFIED_PROMPT_ROLE = "image_5_0_unified"


def render_image_director_template(
    role: str, template_content: str, variables: dict[str, str]
) -> str:
    """Render an Image director prompt with role-scoped substitution rules.

    Only the Image 5.0 unified body role carries the confirmed Manual
    Requirement inside its methodology template, so only that role drops the
    sequential replacement passes that would otherwise rewrite user-inserted
    content.  Every other Image role keeps the established behavior.
    """
    if role == IMAGE_5_0_UNIFIED_PROMPT_ROLE:
        return render_template_string_single_pass(template_content, variables)
...
def run_native_image_three_zero_director(
    ...
    *,
    ...
    if role not in {"image_designer", "image_palette_extractor"}:
        raise ValueError("Native Image Codex text stage role is unsupported")
    operation = "palette_extraction" if role == "image_palette_extractor" else "director"
    failure_code = f"native_image_{operation}_failed"
    if agent_config.get("api_type") != "codex_exec":
        raise ValueError("Native Image 3.0 Director requires a Codex execution profile")
    if configured_model not in _native_image_director_models_for_route(route, role):
    ...
    try:
        execution = run_codex_async_from_sync(
            lambda: run_supervised_codex_child(
                ...
                role=role,
                idempotency_key=(
                    f"codex-native-image-{operation}:run:{run_id}:slide:{run_slide_id}:stage:{stage_id}"
                ),
                invoke=invoke,
        ...
        if not director_final_text.strip():
            raise RuntimeError(f"native_image_{operation}_empty_final")
        ...
        _write_codex_request_evidence(
            result,
            prompt=prompt,
            prompt_skeleton={"role": role, "rendered_prompt_sha256": _sha256_text(prompt)},
        ...
        if not execution.complete_projection():
            raise RuntimeError(f"native_image_{operation}_projection_failed")
        ...
        if execution is not None:
            execution.fail_unrecoverable(failure_code)
...
def run_native_image_palette_extractor(
    agent_config: dict,
    image_path: str | Path,
    *,
    run_id: int,
    run_slide_id: int,
    stage_id: str = "cover-palette-extraction",
    timeout_seconds: int,
) -> tuple[str, list[dict[str, object]], dict[str, object]]:
    """Extract validated palette XML through the audited Codex text boundary.

    Image 5.0 reuses the bound Sol Low director profile as its palette
    extractor.  The source image, prompt and validated XML stay bound to the
    one recorded Codex invocation and its Run-private audit detail.
    """
    import db as dbmod
    ...
    if not source_image.is_file() or source_image.stat().st_size <= 0:
        raise RuntimeError("native_image_palette_source_invalid")
    prompt = color_extraction.PROMPT_PATH.read_text(encoding="utf-8")
    final_text, attempts = run_native_image_three_zero_director(
        agent_config,
        prompt,
        route="image_5_0",
        run_id=run_id,
        ...
        role="image_palette_extractor",
    )
    invocations = [
        invocation
        for invocation in dbmod.list_codex_invocations(
            run_id=run_id,
            run_slide_id=run_slide_id,
            include_events=False,
        )
        if invocation.get("stage_id") == stage_id
        and invocation.get("role") == "image_palette_extractor"
    ]
    if len(invocations) != 1:
        raise RuntimeError("native_image_palette_audit_unavailable")
    invocation = invocations[0]
    detail = codex_audit.get_native_codex_audit_detail(
        run_id=run_id,
        invocation_id=int(invocation["id"]),
    )
    if (
        not isinstance(detail.get("prompt"), str)
        or not isinstance(detail.get("assistant_output"), str)
        or invocation.get("prompt_sha256") != _sha256_text(prompt)
        or invocation.get("output_sha256") != _sha256_text(final_text)
        or (detail.get("lineage") or {}).get("run_slide_id") != run_slide_id
        or (detail.get("lineage") or {}).get("stage_id") != stage_id
    ):
        raise RuntimeError("native_image_palette_audit_unavailable")
    configured_model = str(agent_config.get("model") or "")
    evidence: dict[str, object] = {
        "invocation_id": int(invocation["id"]),
        "audit_detail_endpoint": (
            f"/api/runs/{run_id}/codex-audit/invocations/{int(invocation['id'])}"
        ),
        "configured_model": configured_model,
        "observed_model": configured_model,
        "observed_model_source": "verified_codex_command_identity",
        "requested_reasoning_effort": NATIVE_IMAGE_3_0_DIRECTOR_REASONING_EFFORT,
        "observed_reasoning_effort": NATIVE_IMAGE_3_0_DIRECTOR_REASONING_EFFORT,
        "audit_status": invocation.get("status"),
        "audit_prompt_sha256": invocation.get("prompt_sha256"),
        "audit_output_sha256": invocation.get("output_sha256"),
        "audit_raw_jsonl_sha256": invocation.get("raw_jsonl_sha256"),
        "audit_observed_jsonl_sha256": invocation.get("observed_jsonl_sha256"),
        "audit_started_at": invocation.get("started_at"),
        "audit_ended_at": invocation.get("ended_at"),
        "audit_elapsed_ms": invocation.get("elapsed_ms"),
        "audit_event_count": invocation.get("event_count"),
        "audit_error_event_count": invocation.get("error_event_count"),
        "audit_detail_verified": True,
        "prompt_path": str(color_extraction.PROMPT_PATH),
        "prompt_sha256": _sha256_text(prompt),
        "source_image_path": str(source_image),
        "source_image_sha256": _sha256_path(source_image),
        "source_image_mime_type": mimetypes.guess_type(source_image.name)[0]
        or "image/png",
    }
    try:
        validated = color_extraction.validate_palette_xml(final_text)
    except color_extraction.ColorExtractionError as exc:
        evidence.update(
            {
                "validation_status": "failed",
                "validation_error": str(exc),
                "response_text_sha256": _sha256_text(final_text),
            }
        )
        # The public ColorExtractionError carries no evidence channel; bind the
        # captured stage evidence to the raised instance for the caller.
        setattr(exc, "provider_evidence", evidence)
        raise
    evidence.update(
        {
            "validation_status": "completed",
            "validated_xml_sha256": _sha256_text(validated),
            "validated_xml_content_length": len(validated),
        }
    )
    return validated, attempts, evidence
...
def image_prompt_role(strategy: str, run_slide: dict, seed_slide: dict | None, cover_slide: dict | None) -> str:
    ...
    if strategy == "image_5_0":
        return IMAGE_5_0_UNIFIED_PROMPT_ROLE
...
def _render_image_prompt_with_lineage(
    ...
    *,
    ...
    rendered_prompt = render_image_director_template(
        role,
        prompt["content"],
        {
...
def run_image_route(context) -> ImageRouteOutcome:
    ...
    palette_stage_evidence: dict[str, object] | None = None
    ...
    native_route_metadata = route_metadata.get("native_image")
    native_config_route = model_profiles.native_image_route_for_config(context.config_row)
    native_director_routes = {
        model_profiles.NATIVE_IMAGE_3_0_ROUTE,
        model_profiles.NATIVE_IMAGE_5_0_ROUTE,
    }
    native_three_zero_director = (
        strategy in native_director_routes
        and route_metadata.get("image_renderer") == model_profiles.NATIVE_IMAGE_ADAPTER
        and native_route_metadata
        == {"adapter": model_profiles.NATIVE_IMAGE_ADAPTER, "route": strategy}
        and native_config_route == strategy
        and context.image_designer_config.get("api_type") == model_profiles.CODEX_EXEC_API_TYPE
        and context.image_designer_config.get("model")
        in _native_image_director_models_for_route(strategy, "image_designer")
    ...
    if native_three_zero_requested and not native_three_zero_renderer:
        native_route_label = (
            "5.0"
            if strategy == model_profiles.NATIVE_IMAGE_5_0_ROUTE
            else "3.0"
        )
        raise ValueError(
            f"Native Image {native_route_label} Director requires the server-resolved "
            f"Native Image {native_route_label} configuration"
        )
    ...
    def process_cover_slide() -> None:
        ...
        if strategy == model_profiles.NATIVE_IMAGE_5_0_ROUTE:
            # The established cover methodology predates the 5.0 unified body
            # prompt and has no Deck-User-Requirement slot. Append the already
            # confirmed value after template rendering so Manual requirements
            # reach the cover byte-for-byte and template-like user text cannot
            # be interpreted as a second template pass.
            prompt = (
                f'{prompt}\n\nDeck-wide design instruction requirement:\n"""\n'
        ...
        try:
            if native_three_zero_director:
                cover_prompt_raw, cover_attempts = run_native_image_three_zero_director(
                    ...
                    timeout_seconds=context.timeout_seconds,
                    **({"route": "image_5_0"} if strategy == "image_5_0" else {}),
                )
    ...
    if strategy != "image_3_0":
        try:
            ...
            if strategy == "image_5_0":
                reason = f"blocked_by_image_5_0_cover_failed: {cover_err}"
                current_by_id = {
                    int(slide["id"]): slide
                    for slide in dbmod.list_run_slides(run["id"])
                }
                for content_slide in content_slides:
                    current = current_by_id.get(int(content_slide["id"]), content_slide)
                    if run_status.is_terminal(current.get("status")):
                        continue
                    failure_artifacts = {
                        "terminalization": {
                            "status": run_status.FAILED,
                            "reason": reason,
                            "source": "image_5_0_cover",
                        },
                        "evidence_health": "failed",
                    }
                    dbmod.update_run_slide(
                    ...
                    stage_artifacts[f"slide_{content_slide['position']}"] = failure_artifacts
                dbmod.update_run(
                    run["id"],
                    stage_artifacts=json.dumps(stage_artifacts, ensure_ascii=False),
                )
    ...
    if strategy == "image_5_0" and generated_cover_image_path and not str(effective_color.get("content") or "").strip():
        native_palette_audit = native_three_zero_renderer
        palette_stage_evidence = None
        palette_request_path = os.path.join(output_dir, "cover_palette_request.json")
        palette_response_path = os.path.join(output_dir, "cover_palette_response.json")
        try:
            from backend.services import color_extraction

            codex_palette_extraction = (
                native_palette_audit
                and context.image_palette_extractor_config.get("api_type") == "codex_exec"
            )
            if codex_palette_extraction:
                if cover_slide is None:
                    raise RuntimeError("native_image_palette_cover_slide_unavailable")
                extracted_palette, _palette_attempts, palette_evidence = (
                    run_native_image_palette_extractor(
                        context.image_palette_extractor_config,
                        generated_cover_image_path,
                        run_id=int(run["id"]),
                        run_slide_id=int(cover_slide["id"]),
                        timeout_seconds=context.timeout_seconds,
                    )
                )
            else:
                # Preserve the existing product route's exact public call shape.
                extracted_palette = color_extraction.extract_palette_xml_from_file(
                    generated_cover_image_path,
            ...
            if native_palette_audit:
                required_evidence = (
                    ...
                    "audit_prompt_sha256",
                    "audit_output_sha256",
                    "audit_detail_verified",
                    "prompt_path",
                    "prompt_sha256",
                    "source_image_path",
                    "source_image_sha256",
                    "validated_xml_sha256",
                )
                palette_health = (
                    "complete"
                    if all(palette_evidence.get(key) is not None for key in required_evidence)
                    else "incomplete"
                )
                palette_request = {
                    "role": "image_palette_extractor",
                    "profile_id": context.image_palette_extractor_config.get("profile_id"),
                    "profile_name": context.image_palette_extractor_config.get("profile_name")
                    or context.image_palette_extractor_config.get("name"),
                    "api_type": context.image_palette_extractor_config.get("api_type"),
                    "api_key": "[REDACTED]",
                    **{
                        key: palette_evidence.get(key)
                        for key in (
                            "invocation_id",
                            "audit_detail_endpoint",
                            "configured_model",
                            ...
                            "audit_prompt_sha256",
                            "audit_output_sha256",
                            "audit_raw_jsonl_sha256",
                            "audit_observed_jsonl_sha256",
                            "audit_started_at",
                            "audit_ended_at",
                            "audit_elapsed_ms",
                            "audit_event_count",
                            "audit_error_event_count",
                            "audit_detail_verified",
                        )
                    },
                }
                Path(palette_request_path).write_text(
                    json.dumps(palette_request, ensure_ascii=False, indent=2),
                    encoding="utf-8",
                )
                Path(palette_response_path).write_text(
                    json.dumps(
                        {
                            "status": "completed",
                            "palette_xml": extracted_palette,
                            "content_length": len(extracted_palette),
                            "evidence_health": palette_health,
                            **palette_evidence,
                        },
                        ensure_ascii=False,
                        indent=2,
                ...
                palette_stage_evidence = request_chain_stage_evidence(
                    stage_id="cover-palette-extraction",
                    stage_name="Cover Palette Extraction",
                    role="image_palette_extractor",
                    prompt_role="image_palette_extractor",
                    model_config=context.image_palette_extractor_config,
                    prompt_path=palette_evidence.get("prompt_path"),
                    request_path=palette_request_path,
                    response_path=palette_response_path,
                    artifact_path=palette_response_path,
                    health=palette_health,
                    extra={
                        "source_run_slide_id": cover_slide.get("id") if cover_slide else None,
                        **palette_evidence,
                    },
                )
            ...
            failed_palette_stage = None
            if native_palette_audit:
                palette_evidence = dict(
                    getattr(palette_err, "provider_evidence", None) or {}
                )
                Path(palette_request_path).write_text(
                    json.dumps(
                        {
                ...
                Path(palette_response_path).write_text(
                    json.dumps(
                        {"status": "failed", "error": str(palette_err), **palette_evidence},
                        ensure_ascii=False,
                        indent=2,
                    ),
                    encoding="utf-8",
                )
                palette_failure.update(
                    {
                        "profile_id": context.image_palette_extractor_config.get("profile_id"),
                        "model": context.image_palette_extractor_config.get("model"),
                        "request_path": palette_request_path,
                ...
                failed_palette_stage = request_chain_stage_evidence(
                    stage_id="cover-palette-extraction",
                    stage_name="Cover Palette Extraction",
                    role="image_palette_extractor",
                    prompt_role="image_palette_extractor",
                    ...
                    artifact_path=palette_response_path,
                    health="failed",
                    extra={"error": str(palette_err), **palette_evidence},
                )
            stage_artifacts["cover_palette"] = palette_failure
                ...
                if index == 0:
                    failure_artifacts = {
                        ...
                        "request_chain": request_chain_evidence(
                            ...
                            stages=(
                                [failed_palette_stage]
                                if failed_palette_stage is not None
                                else None
                            ),
                    ...
                    skipped_artifacts = {
                        ...
                        "request_chain": request_chain_evidence(
                            ...
                            prompt_role=image_prompt_role(strategy, blocked_slide, seed_slide, cover_slide),
                            planned_chain=(
                                ["cover_palette_extraction", "content_designer", "content_image_generation"]
                                if native_palette_audit
                                else ["content_designer", "content_image_generation"]
                            ),
                            actual_evidence=(
                                {
                                    "blocked_by": "palette_extraction_failed",
                                    "cover_palette": stage_artifacts["cover_palette"],
                                }
                                if native_palette_audit
                                else {"blocked_by": "palette_extraction_failed"}
                            ),
                            stages=(
                                [failed_palette_stage]
                                if failed_palette_stage is not None
                            ...
                            palette=(
                                {"status": "failed", "source": "cover", **palette_evidence}
                                if native_palette_audit
                                else {"status": "skipped", "source": "cover"}
                            ),
                            health="skipped",
    ...
    def process_content_slide(rs: dict) -> None:
        ...
        try:
            ...
            if strategy == "image_5_0" and palette_stage_evidence:
                palette_state = {
                    "status": "used",
                    "source": "cover",
                    "source_run_slide_id": palette_stage_evidence.get(
                        "source_run_slide_id"
                    ),
                    "source_image_path": palette_stage_evidence.get(
                        "source_image_path"
                    ),
                    "sent_to_model": True,
                    "cover_png_sent": False,
                    "content_length": len(str(effective_color.get("content") or "")),
                    "profile_id": palette_stage_evidence.get("profile_id"),
                    "model": palette_stage_evidence.get("model"),
                    "request_path": palette_stage_evidence.get("request_path"),
                    "response_path": palette_stage_evidence.get("response_path"),
                    "raw_response_path": palette_stage_evidence.get(
                        "raw_response_path"
                    ),
                    "evidence_health": palette_stage_evidence.get("health"),
                    "configured_endpoint": palette_stage_evidence.get(
                        "configured_endpoint"
                    ),
                    "request_url": palette_stage_evidence.get("request_url"),
                    "configured_model": palette_stage_evidence.get(
                        "configured_model"
                    ),
                    "observed_model": palette_stage_evidence.get(
                        "observed_model", "unknown"
                    ),
                    "observed_model_source": palette_stage_evidence.get(
                        "observed_model_source", "configured_only"
                    ),
                    "prompt_sha256": palette_stage_evidence.get("prompt_sha256"),
                    "source_image_sha256": palette_stage_evidence.get(
                        "source_image_sha256"
                    ),
                    "request_body_sha256": palette_stage_evidence.get(
                        "request_body_sha256"
                    ),
                    "http_status": palette_stage_evidence.get("http_status"),
                    "provider_request_id": palette_stage_evidence.get(
                        "provider_request_id"
                    ),
                    "raw_response_sha256": palette_stage_evidence.get(
                        "raw_response_sha256"
                    ),
                    "started_at": palette_stage_evidence.get("started_at"),
                    "ended_at": palette_stage_evidence.get("ended_at"),
                    "elapsed_ms": palette_stage_evidence.get("elapsed_ms"),
                    "validated_xml_sha256": palette_stage_evidence.get(
                        "validated_xml_sha256"
                    ),
                }
            if (
