"""Command-line entrypoint for the fixed public Image PPT 5.0 workflow."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Public Image PPT 5.0 base URL",
    )
    ...
    revise.add_argument("--allow-title-changes", action="store_true")
    ...
    generate = subcommands.add_parser(
        "generate", help="Start fixed Image 5.0 generation"
    )
    generate.add_argument("--deck-id", required=True, type=int)
    generation_intent = generate.add_mutually_exclusive_group(required=True)
    generation_intent.add_argument(
        "--auto",
        action="store_true",
        help="Delegate the whole Deck design to the fixed Auto path",
    )
    generation_intent.add_argument(
        "--requirement-file",
        type=Path,
...
def _read_requirement_text(path: Path) -> str:
    """Read the confirmed Deck direction without trimming or rewriting bytes."""
    payload = path.read_bytes()
    text = payload.decode("utf-8")
    if not text.strip():
        raise ValueError("requirement_empty")
    return text
