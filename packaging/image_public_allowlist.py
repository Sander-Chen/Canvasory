...
IMAGE_PROMPT_SOURCE_FILES = (
    ...
    "example/prompts/image-5-0-unified.md",
    "example/prompts/faithful-split.md",
    "example/prompts/palette-extraction.md",
)

# The Image Auto lane reads this exact established AutoSkill document at run
# time, so the public runtime bundle and public source snapshot must ship it
# byte-for-byte instead of failing on an absent file.
IMAGE_AUTO_SKILL_SOURCE_FILES = (
    "example/auto-skill-5.3.20.md",
)
...
def required_runtime_source_paths() -> tuple[str, ...]:
    """Return every exact source file the public Image runtime reads."""
    return required_prompt_relative_paths() + IMAGE_AUTO_SKILL_SOURCE_FILES
...
def runtime_relative_paths(repo_root: Path) -> list[str]:
    ...
    for relative in APP_PYTHON_FILES + required_runtime_source_paths():
