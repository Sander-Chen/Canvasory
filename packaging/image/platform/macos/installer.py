...
class ProvisionResult(NamedTuple):
    ...
    durable_runtime_root: Path | None = None
    durable_python: Path | None = None
...
class FallbackAuthorization(NamedTuple):
    ...
    def durable_record(self, runtime_root: Path, python_path: Path) -> dict[str, Any]:
        """Bind the authorization record to the installed durable Runtime copy.

        The receipt binds the bootstrap's temporary extraction directory, which
        is deleted when the bootstrap exits.  Installations record the copy the
        venv is actually built from so the marker remains verifiable and does
        not reference the removed bootstrap path.
        """
        record = self.record()
        record["fallback_runtime"]["extracted_root"] = str(runtime_root)
        record["fallback_runtime"]["python_path"] = str(python_path)
        return record
...
def _copy_runtime_symlink(member: Path, source: Path, destination: Path) -> None:
    ...
    if (
        ...
        raise InstallerError(
            "fallback_runtime_unsafe",
            "frozen fallback Runtime contains a link outside its payload",
        )
    normalized = os.path.normpath(
        os.path.join(os.fspath(member.parent.relative_to(source)), target)
    )
...
def _copy_runtime_tree(source: Path, staged: Path) -> None:
    """Copy the frozen Runtime into the install root as a self-contained tree.

    Relative links that stay inside the payload are preserved, mirroring the
    bootstrap's frozen-archive safety validation.  Members that would leave the
    tree, and members that are not files, directories, or links, are rejected
    so the durable copy cannot depend on the bootstrap's temporary directory.
    """
    try:
        ...
        for current, directories, files in os.walk(source, followlinks=False):
            ...
            for name in (*directories, *files):
                ...
                destination = staged / relative / name
                if member.is_symlink():
                    _copy_runtime_symlink(member, source, destination)
                elif member.is_dir():
                    destination.mkdir()
                elif member.is_file():
                    ...
                    raise InstallerError(
                        "fallback_runtime_unsafe",
                        "frozen fallback Runtime contains an unsupported member",
                    )
    except OSError as exc:
        raise InstallerError(
            "fallback_runtime_copy_failed",
            "frozen fallback Runtime could not be copied into the installation",
        ) from exc


def _durable_fallback_runtime(
    authorization: FallbackAuthorization,
    layout: InstallLayout,
    install_id: str,
) -> tuple[Path, Path]:
    """Place the authorized frozen Runtime inside the user-scoped install root.

    ``python -m venv`` records its base interpreter in ``pyvenv.cfg``, so the
    frozen Runtime must live at a durable path *before* the venv is created.
    The copy is staged, probed from its durable location, and activated with
    one atomic rename.
    """
    destination = layout.runtimes / install_id
    staged = layout.staging / f"fallback-runtime-{install_id}-{uuid.uuid4().hex}"
    try:
        _copy_runtime_tree(authorization.fallback_root, staged)
        python = _fallback_executable(staged)
        try:
            _probe_python(python)
        except InstallerError as exc:
            raise InstallerError(
                "fallback_runtime_unavailable",
                "the durable frozen Runtime copy did not run from the installation",
                cause_code=exc.code,
            ) from exc
        _quarantine_incomplete(layout, destination, f"runtime-{install_id}")
        os.replace(staged, destination)
    except OSError as exc:
        raise InstallerError(
            "fallback_runtime_copy_failed",
            "frozen fallback Runtime could not be made durable",
        ) from exc
    finally:
        shutil.rmtree(staged, ignore_errors=True)
    return destination, _fallback_executable(destination)


def _recorded_fallback_runtime(record: Mapping[str, Any], layout: InstallLayout) -> Path:
    value = record.get("fallback_runtime_path")
    if not isinstance(value, str):
        raise InstallerError(
            "fallback_runtime_missing",
            "fallback installation has no durable frozen Runtime record",
        )
    try:
        root = Path(value).expanduser().resolve(strict=True)
        root.relative_to(layout.root)
    except (OSError, RuntimeError, ValueError) as exc:
        raise InstallerError(
            "fallback_runtime_missing",
            "fallback installation durable frozen Runtime is unavailable",
        ) from exc
    if not root.is_dir():
        raise InstallerError(
            "fallback_runtime_missing",
...
def _venv_bound_to_runtime(venv: Path, runtime_root: Path) -> bool:
    """True when pyvenv.cfg records its base interpreter inside runtime_root."""
    try:
    ...
    if not home:
        return False
    for candidate in (home, values.get("executable")):
        if not candidate:
            continue
        try:
            Path(candidate).expanduser().resolve().relative_to(runtime_root)
...
def _durable_fallback(
    authorization: FallbackAuthorization | None,
    *,
    ...
    if layout is None or install_id is None:
        raise InstallerError(
            "fallback_runtime_unavailable",
            "durable frozen Runtime destination is unavailable",
        )
    return _durable_fallback_runtime(authorization, layout, install_id)
...
def _provision_with_fallback(
    ...
    *,
    ...
    for approach in approaches:
        ...
        try:
            ...
            if runtime.source == "fallback":
                durable_runtime_root, durable_python = _durable_fallback(
                    fallback_authorization,
                    layout=layout,
                    install_id=install_id,
                )
                candidate = RuntimeChoice("fallback", durable_python, runtime.version)
            _provision_venv(
                candidate,
                venv_root,
                app_root,
    ...
    try:
        ...
        fallback = RuntimeChoice("fallback", durable_python, fallback.version)
        shutil.rmtree(venv_root, ignore_errors=True)
        _provision_venv(
            fallback,
            venv_root,
...
def install_release(
    *,
    ...
    if (
        ...
        and completion.get("runtime_source") == "fallback"
    ):
        # A frozen-Runtime installation is reusable only while the durable
        # Runtime copy recorded at install time is present and the venv is
        # still bound to it.  A record from an older installer, or a venv that
        # still points at the removed bootstrap directory, is repaired through
        # the full install path below.
        try:
            fallback_runtime_root = _recorded_fallback_runtime(completion, layout)
            fallback_python = _fallback_executable(fallback_runtime_root)
            if not _venv_bound_to_runtime(venv_target, fallback_runtime_root):
                raise InstallerError(
                    "fallback_runtime_unbound",
                    "installed environment is not bound to its durable frozen Runtime",
                )
        except InstallerError:
            reused = False
            fallback_runtime_root = None
    ...
    if reused:
        ...
        if runtime.source == "fallback":
            if fallback_runtime_root is None or fallback_python is None:
                raise InstallerError(
                    "fallback_runtime_missing",
                    "reused fallback release has no durable frozen Runtime",
                )
            ...
            if completion.get("fallback_authorization") != (
                fallback_authorization.durable_record(
                    fallback_runtime_root, fallback_python
                )
            ):
                raise InstallerError(
                    "fallback_not_authorized",
    ...
    fallback_runtime_path: str | None = None
    fallback_record: dict[str, Any] | None = None
    if reused and runtime.source == "fallback":
        if (
            fallback_authorization is None
            or fallback_runtime_root is None
            or fallback_python is None
        ):
            raise InstallerError(
                "fallback_runtime_missing",
                "reused fallback release has no durable frozen Runtime",
            )
        fallback_runtime_path = str(fallback_runtime_root)
        fallback_record = fallback_authorization.durable_record(
            fallback_runtime_root, fallback_python
        )
    ...
    try:
        if not reused:
            ...
            if runtime.source == "fallback":
                ...
                fallback_runtime_root = provision.durable_runtime_root
                fallback_python = provision.durable_python
                if fallback_runtime_root is None or fallback_python is None:
                    raise InstallerError(
                        "fallback_runtime_missing",
                        "fallback durable frozen Runtime is unavailable",
                    )
                fallback_runtime_path = str(fallback_runtime_root)
                fallback_record = fallback_authorization.durable_record(
                    fallback_runtime_root, fallback_python
            ...
            install_record = {
                ...
                "fallback_authorization": fallback_record,
            }
        ...
        if not targets_complete:
            ...
            _quarantine_incomplete(
                layout, layout.runtimes / install_id, f"runtime-{install_id}"
            )
        raise
    finally:
