"""Fail-closed public HTTP surface for the fixed Image 5.0 product."""
...
_PUBLIC_GENERATE_MANUAL_FIELDS = {
    "deck_id",
    "mode",
    "requirement_text",
}
...
def _public_native_profile_matches(
    ...
    *,
    ...
    return bool(
        ...
        and profile.get("api_type") == api_type
        and profile.get("endpoint") == model_profiles.CODEX_EXEC_ENDPOINT
        and profile.get("model") == model
        and profile.get("thinking") == _PUBLIC_IMAGE_5_0_THINKING
        and profile.get("status") == "active"
        and float(profile.get("temperature") or 0) == 1.0
...
def _public_image_5_0_config(name: str = _PUBLIC_IMAGE_5_0_CONFIG_NAME) -> dict[str, Any] | None:
    ...
    if len(rows) != 1:
        return None
    try:
        config = model_profiles.resolve_config(int(rows[0]["id"]))
    ...
    if (
        ...
        or model_profiles.native_image_route_for_config(config)
        != _PUBLIC_IMAGE_5_0_STRATEGY
    ):
        return None
    ...
    for role in ("image_designer", "image_generator", "image_palette_extractor"):
        ...
        if (
            ...
            return None
        profile_ids[role] = int(binding["profile_id"])
    profiles = {
...
def _run_is_public(run: dict[str, Any] | None) -> bool:
    return bool(
        ...
        and run.get("strategy") in _PUBLIC_RUN_STRATEGIES
...
def _public_manifest_route(runs: list[dict[str, Any]]) -> tuple[str, str]:
    """Name one ZIP manifest from the actual public run strategy."""
    if runs and all(run.get("strategy") == _PUBLIC_IMAGE_5_0_STRATEGY for run in runs):
        return "public_image_5_0", _PUBLIC_IMAGE_5_0_STRATEGY
    return _IMAGE_RUNTIME_SURFACE, model_profiles.NATIVE_IMAGE_3_0_ROUTE


def _public_download_route_name(strategy: str) -> str:
    """Keep legacy 3.0 filenames while naming 5.0 archives by their route."""
    if strategy == _PUBLIC_IMAGE_5_0_STRATEGY:
        return "image-5-0"
    return "image-3-0"
...
def _public_run_download(run_id: int, artifacts_root: Path):
    ...
    surface, strategy = _public_manifest_route([run])
    buffer = BytesIO()
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as archive:
        ...
        manifest = {
            ...
            "route": {"engine": "image", "strategy": strategy},
            "included": included,
        }
        archive.writestr("manifest.json", json.dumps(manifest, ensure_ascii=False, indent=2))
    route_name = _public_download_route_name(strategy)
    return _zip_response(buffer, f"run-{run_id}-{route_name}.zip")
...
def _public_batch_download(batch_id: int, artifacts_root: Path):
    ...
    surface, strategy = _public_manifest_route(runs)
    buffer = BytesIO()
    included: list[dict[str, Any]] = []
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as archive:
        for run in runs:
        ...
        archive.writestr("manifest.json", json.dumps(manifest, ensure_ascii=False, indent=2))
    route_name = _public_download_route_name(strategy)
    return _zip_response(buffer, f"batch-{batch_id}-{route_name}.zip")
...
def _public_slide_evidence_download(run_slide_id: int, artifacts_root: Path):
    ...
    run = dbmod.get_run(int(slide["run_id"]))
    if run is None:
        return _not_found()
    surface, _strategy = _public_manifest_route([run])
    manifest = {
        "surface": surface,
...
def _public_launch_batch_runs():
    """Use the same runtime launcher seam as the shared Generate view."""
    try:
        import server as server_module
    except ImportError:  # pragma: no cover - direct service import without a server
        return generation.default_launch_batch_runs
    return server_module.launch_batch_runs
...
def _public_split_visible_text(value: object) -> str:
    return " ".join(str(value or "").split())


def _public_split_title_integrity(
    deck_content: str,
    slides: list[dict[str, str]],
    *,
    allow_title_changes: bool,
    prefer_explicit_h1: bool,
) -> None:
    """Keep source structural titles visible unless the typed route allows edits.

    Only real Markdown structural units carry source titles; plain-text units
    receive synthetic content-derived titles and stay out of scope.  Each
    original title must appear, in source order, across the revised slide titles
    and bodies, so the observed all-generic-title plan fails closed while an
    explicitly authorized title-edit route keeps its supported path.  The
    server never guesses this authority from natural-language wording.
    """
    if allow_title_changes:
        return
    if not deck_split_drafts._markdown_structural_units(
        deck_content, prefer_explicit_h1=prefer_explicit_h1
    ):
        return
    expected = [
        deck_split_drafts._section_title(unit.get("title"))
        for unit in deck_split_drafts._ordered_source_units(
            deck_content, prefer_explicit_h1=prefer_explicit_h1
        )
    ]
    ...
    if not expected:
        return
    visible = "\n".join(
        f"{deck_split_drafts._section_title(slide.get('title'))}\n"
        f"{_public_split_visible_text(slide.get('content'))}"
        for slide in slides
    )
    cursor = 0
    for title in expected:
        index = visible.find(title, cursor)
        if index < 0:
            raise deck_split_drafts.SplitDraftError(
                "Auto Split integrity check failed: public faithful source titles "
                "must stay visible and ordered unless the instruction changes titles"
            )
...
def _public_revise_split_draft(
    ...
    *,
    ...
    if type(allow_title_changes) is not bool or (
        ...
        raise deck_split_drafts.SplitDraftError(
            "allow_title_changes requires an instruction revision"
        )
...
def _request_is_allowed(artifacts_root: Path):
    ...
    if rule == "/api/deck-split-drafts/<int:draft_id>/revise":
        ...
        valid_instruction = (
            payload_keys in ({"instruction"}, {"instruction", "allow_title_changes"})
            and isinstance(instruction, str)
            and bool(instruction.strip())
            and (
                "allow_title_changes" not in payload
                or payload.get("allow_title_changes") is True
            )
        )
...
def _install_public_read_views(app, artifacts_root: Path) -> None:
        ...
        try:
            if "target_page_count" in payload:
                ...
                draft = _public_revise_split_draft(
                    ...
                    allow_title_changes=payload.get("allow_title_changes") is True,
                )
        except Exception as error:
            return _public_split_error(error)
    ...
    def generate():
        """Create one fixed Image 5.0 batch from a validated public intent."""
        payload = request.get_json(silent=True)
        ...
        data: dict[str, Any] = {
            ...
            "engine": "image",
            "strategy": _PUBLIC_IMAGE_5_0_STRATEGY,
        }
        if payload["mode"] == "auto":
            data["mode"] = "auto"
        else:
            # The confirmed direction is stored as one exact Requirement before
            # the existing Manual plan is asked to accept it.  A later rejection
            # keeps this truthful record and is never retried here.
            requirement_id = dbmod.create_requirement(
                _PUBLIC_IMAGE_5_0_REQUIREMENT_TITLE,
                payload["requirement_text"],
            )
            data["requirement_ids"] = [requirement_id]
            data["color_ids"] = []
        try:
            result = generation.create_generation_batch(
                data,
                db_path=str(dbmod.DB_PATH),
                launch_batch_runs=_public_launch_batch_runs(),
                batch_run_limit=generation.DEFAULT_BATCH_RUN_LIMIT,
            )
