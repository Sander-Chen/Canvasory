...
AUTO_SKILL_SHA256 = "654c6398a03dbdd95076eecbd245f4985e1ed0468aec647cb1ba18d25e9129f6"
...
def _read_auto_skill_content() -> str:
    """Read the exact AutoSkill document, failing closed without substitution."""
    try:
        payload = AUTO_SKILL_PATH.read_bytes()
        content = payload.decode("utf-8")
    except (OSError, UnicodeError) as exc:
        raise AutoGenerationRequestError("AutoSkill source is unavailable", 503) from exc
    if not content.strip() or hashlib.sha256(payload).hexdigest() != AUTO_SKILL_SHA256:
        raise AutoGenerationRequestError("AutoSkill source is unavailable", 503)
    return content


def get_or_create_auto_requirement() -> int:
    content = _read_auto_skill_content()
...
def create_auto_generation_batch(
    ...
    # Bind the same confirmed cover snapshot the Manual plan already captures,
    # before any Run exists, so a later Deck edit cannot change Auto inputs.
    from backend.services.generation import _image_skill_cover_metadata
